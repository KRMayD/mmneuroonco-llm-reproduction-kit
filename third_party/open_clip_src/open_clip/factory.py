...
def load_state_dict(
    ...
    if str(checkpoint_path).endswith(".safetensors"):
        ...
        try:
            ...
            # Fallback for older checkpoints with numpy scalars or other unpickling issues
...
def create_loss(args):
    ...
    elif getattr(args, 'dpo_loss', False):
        if getattr(args, "dpo_mode", "standard") == "atomic":
            from .loss import AtomicDpoLoss
            return AtomicDpoLoss(
                atomic_loss_preset=getattr(args, "atomic_loss_preset", "full"),
                beta=getattr(args, "beta_dpo", 0.1),
                normalize_features=not getattr(args, "disable_feature_normalization", False),
                lambda_global=getattr(args, "lambda_global", 1.0),
                global_fourway_weights=getattr(args, "global_fourway_weights", None),
                lambda_atomic_fourway=getattr(args, "lambda_atomic_fourway", 0.0),
                atomic_fourway_weights=getattr(args, "atomic_fourway_weights", None),
                lambda_atomic_text=getattr(args, "lambda_atomic_text", 1.0),
                lambda_atomic_visual=getattr(args, "lambda_atomic_visual", 0.5),
                use_neg_image_for_atomic_visual=getattr(args, "atomic_visual_use_neg_image", True),
                atomic_visual_neg_source=getattr(args, "atomic_visual_neg_source", "global_neg"),
                use_global_4way=getattr(args, "dpo_loss_type", "4way") == "4way",
                atomic_train_mode=getattr(args, "atomic_train_mode", "row_all"),
                num_atomic_attrs=getattr(args, "num_atomic_attrs", 6),
                scale_global_in_prop_sample=getattr(args, "scale_global_in_prop_sample", False),
                atomic_dropout_k=getattr(args, "atomic_dropout_k", 0),
                atomic_dropout_p=getattr(args, "atomic_dropout_p", 0.0),
                attr_weights=getattr(args, "attr_loss_weights", None),
                atomic_text_weight_mode=getattr(args, "atomic_text_weight_mode", "uniform"),
                semantic_weight_scale=getattr(args, "semantic_weight_scale", 1.0),
                semantic_weight_min=getattr(args, "semantic_weight_min", 0.5),
                semantic_weight_max=getattr(args, "semantic_weight_max", 2.0),
                hard_mining=getattr(args, "hard_mining", "none"),
                hard_temperature=getattr(args, "hard_temperature", 0.1),
                hard_topk=getattr(args, "hard_topk", 0),
            )
        from .loss import DpoLoss
        return DpoLoss(
            beta=getattr(args, 'beta_dpo', 0.1),
            loss_type=getattr(args, 'dpo_loss_type', '4way'),
            fourway_weights=getattr(args, 'global_fourway_weights', None),
            normalize_features=not getattr(args, "disable_feature_normalization", False),
        )
    return ClipLoss(
        local_loss=args.local_loss,
