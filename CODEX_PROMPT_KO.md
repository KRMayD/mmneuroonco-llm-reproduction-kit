...
목표는 upstream vision encoder만 바꾸고 나머지 downstream 조건을 완전히
동일하게 고정한 네 모델 비교다.
...
3. prepare_mmneuro_official_vqa.py로 /data/mmneuro_assets/mm_neuroonco_official
   manifest를 재생성한다.
4. 사용자에게 전달받은 네 vision checkpoint를 artifacts/vision_checkpoints.tsv의
   SHA256으로 검증한다.
...
7. RUN_ROOT의 protocol.json, 모델별 *.summary.json, metrics_comparison.json,
   logs, 사용한 LLM checkpoint 경로를 검토하고 표로 결과를 보고한다.
